...
import { MOCK_LISTINGS, DEFAULT_SYSTEM_CONFIG, INITIAL_USERS, DEMO_RENTER, DEMO_HUBBER, DEMO_ADMIN } from './constants';
...
const App: React.FC = () => {
  ...
  const handleLogin = (user: User) => {
    setCurrentUser(user);
    // Update allUsers if needed (e.g., new signup)
  ...
  const handleHostClick = (host: User) => {
    // Always find the fresh user object
    const freshHost = allUsers.find(u => u.id === host.id) || host;
      ...
      if (request) {
        ...
        if (targetUser) {
           const updatedTargetUser = {
             ...targetUser,
             hubberBalance: targetUser.hubberBalance - request.amount
           };
           handleUpdateUser(updatedTargetUser);
        }
      }
    }
  };
