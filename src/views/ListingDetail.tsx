...
export const ListingDetail: React.FC<ListingDetailProps> = ({ listing, currentUser, onBack, systemConfig, onPaymentSuccess, onHostClick, allUsers }) => {
  ...
  const currentOwner = allUsers.find(u => u.id === listing.ownerId) || listing.owner;

  // Inject updated owner into listing for display components
  const displayListing = { ...listing, owner: currentOwner! };
  ...
  return (
    <div className="bg-white min-h-screen pb-12 relative font-sans">
              ...
              <div className="p-6 space-y-6">
                 <div className="bg-gray-50 p-4 rounded-xl space-y-2">
                    <div className="flex justify-between text-sm text-gray-600">
                       <span>Noleggio ({duration} {displayListing.priceUnit === 'ora' ? 'ore' : 'notte/i'})</span>
                       <span>€{subtotal}</span>
                    </div>
                    <div className="flex justify-between text-sm text-gray-600">
                    ...
                    {displayListing.deposit && displayListing.deposit > 0 && (
                       <div className="flex justify-between text-sm text-gray-600">
                          <span>Cauzione</span>
                          <span>€{displayListing.deposit}</span>
                       </div>
                    )}
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
         ...
         {/* 1. PHOTO GALLERY */}
         <PhotoGallery images={displayListing.images} />
         ...
         <div className="grid grid-cols-1 lg:grid-cols-3 gap-12 mt-8">
            ...
            <div className="lg:col-span-2 space-y-8">
               ...
               <div className="border-b border-gray-100 pb-6">
                  {/* 2. LISTING BADGES */}
                  <ListingBadges listing={displayListing} />
                  
                  <h1 className="text-3xl font-bold text-gray-900 mb-2">{displayListing.title}</h1>
                  <div className="flex items-center text-gray-600 text-sm">
                     <span className="font-semibold flex items-center text-gray-900 mr-1"><Star className="w-4 h-4 fill-current mr-1" /> {displayListing.rating || 'New'}</span>
                     <span className="mx-2">·</span>
                     <span className="underline">{displayListing.reviewCount} recensioni</span>
                     <span className="mx-2">·</span>
                     <span className="flex items-center"><MapPin className="w-4 h-4 mr-1" /> {displayListing.location}</span>
                  </div>
               </div>
               
               {/* 3. HOST INFO - CLICKABLE */}
               <HostInfo owner={displayListing.owner} onHostClick={onHostClick} />

               {/* Description & Features */}
               <div className="py-4 space-y-6 border-b border-gray-100 pb-8">
                  <p className="text-gray-700 leading-relaxed whitespace-pre-line">{displayListing.description}</p>
                  
                  <div>
                     <h3 className="font-bold text-gray-900 mb-4">Cosa troverai</h3>
                     <div className="grid grid-cols-2 gap-4">
                        {displayListing.features.map((f, i) => (
               ...
               {/* 4. RULES & POLICIES */}
               <RulesAndPolicies rules={displayListing.rules} cancellationPolicy={displayListing.cancellationPolicy} deposit={displayListing.deposit} />

               {/* 5. MAP SECTION */}
               <MapSection location={displayListing.location} coordinates={displayListing.coordinates} />
               
               {/* 6. REVIEWS SECTION */}
               <ReviewsSection rating={displayListing.rating} reviewCount={displayListing.reviewCount} reviews={displayListing.reviews} />
            ...
            <div className="lg:col-span-1 relative">
               <div className="sticky top-28 border border-gray-200 rounded-2xl shadow-xl p-6 bg-white z-30">
                  ...
                  <div className="flex justify-between items-end mb-6">
                     <div>
                        <span className="text-2xl font-bold text-gray-900">€{displayListing.price}</span>
                        <span className="text-gray-500"> / {displayListing.priceUnit}</span>
                     </div>
                     <div className="text-sm text-gray-500 underline cursor-pointer">
                        {displayListing.reviewCount} recensioni
                     </div>
                  </div>
                  ...
                  <div className="border border-gray-400 rounded-xl overflow-visible mb-4 relative bg-white">
                     ...
                     {displayListing.category === 'spazio' && (
                        <>
                     ...
                     {displayListing.priceUnit === 'ora' && displayListing.category === 'oggetto' && (
                     ...
                     <>
                        ...
                        <div className="space-y-3 text-gray-600">
                           <div className="flex justify-between underline decoration-gray-300">
                              <span>€{displayListing.price} x {duration} {displayListing.priceUnit === 'ora' ? 'ore' : 'notti'}</span>
                              <span>€{subtotal}</span>
                           </div>
                           <div className="flex justify-between underline decoration-gray-300">
         ...
         <div className="mt-16 pt-12 border-t border-gray-200">
            <RelatedListingsSection category={displayListing.category} />
         </div>
      </div>
    </div>
  );
